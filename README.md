...
Tired of manually checking jobs? I built Job-Search-Agent. It’s an AI system that finds job links, scrapes and analyses them. It uses a Hybrid RAG pipeline to validate each job against your resume, so you only apply to jobs that matter.
...
5.  **Self-Correction** Implemented a router-based reflection loop to check for possible hallucinations with max_iter=1 to avoid infinite looping.
